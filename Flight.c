...
int saveCompressedFlight(const Flight* pFlight, const FILE* fp) {
	BYTE pBuffer[BIN_FLIGHT_BYTE_SIZE];
	char* srcIATA = pFlight->originCode;
	char* destIATA = pFlight->destCode;
	for (int i = 0;i < 3;i++)
		pBuffer[i] = srcIATA[i];

	for (int i = 3;i < 6;i++)
		pBuffer[i] = destIATA[i - 3];

	compressedDate(pBuffer, &pFlight->date);
	pBuffer[9] = pBuffer[9] | (pFlight->hour) << 3;

	if (!writeToBinFile(fp, pBuffer, BIN_FLIGHT_BYTE_SIZE))
		return 0;
...
int unCompressedFlight(unsigned char* pBuffer, Flight* pFlight, AirportManager* pManager) {
	char srcIATA[CODE_LENGTH + 1];
	char destIATA[CODE_LENGTH + 1];
	for (int i = 0;i < 3;i++) {
		srcIATA[i] = pBuffer[i];
	}
	if (!checkUniqeCode(srcIATA, pManager))
		return 0;
	srcIATA[3] = '\0';

	for (int i = 3;i < 6;i++) {
		destIATA[i - 3] = pBuffer[i];
	}

	if (!checkUniqeCode(destIATA, pManager))
		return 0;
	destIATA[3] = '\0';

	unCompressedDate(pBuffer, &pFlight->date);
	pFlight->hour = pBuffer[9] >> 3;
	strcpy(pFlight->originCode, srcIATA);
	strcpy(pFlight->destCode, destIATA);
	return 1;
}
