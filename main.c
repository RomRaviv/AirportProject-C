...
int main()
{
	...
	int fileFormat=initManagerAndCompany(&manager, &company);
	...
	//saving the current company on both binary files.
	saveCompanyToFile(&company, REGULAR_COMPANY_FILE_NAME);
	saveCompressedCompany(&company, COMPR_COMPANY_FILE_NAME);
...
int initManagerAndCompany(AirportManager* pManager ,Company* pCompany)
{
	...
	if (res == FROM_REG_FILE)
		return initCompanyFromFile(pCompany,pManager, REGULAR_COMPANY_FILE_NAME);
	else if(res==FROM_USER)
		initCompany(pCompany, pManager);
	else
		loadCompressedCompany( pCompany, pManager, COMPR_COMPANY_FILE_NAME);
	return res;
}